...
with col1:
    ...
    with t1:
        st.markdown("**Cloud LLM API** — best quality, works on Streamlit Cloud")
        if _claude_api_key():
        ...
        st.caption(
            "Paste your own cloud provider key to use Tier 1 for just your session. "
...
with col2:
    ...
    st.caption(
        "By default the app auto-picks the best available tier (Cloud LLM > Groq > Ollama > "
        "rule-based) — with a cloud LLM key configured, it always wins. Force a lower tier "
        "here to actually test it, without removing your key or touching secrets.toml."
    )
    _mode_labels = {
        "auto": "Auto (best available)",
        "claude": "Force Cloud LLM",
