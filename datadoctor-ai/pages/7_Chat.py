...
st.set_page_config(page_title="Chat · DataDoctor AI", page_icon="💬", layout="wide")
inject_global_css()
sidebar_brand()
...
with st.sidebar:
    ...
    rag_enabled = st.toggle("Inject RAG context", value=True,
                             help="Retrieve similar past incidents and inject them before each response")
    stream_enabled = st.toggle("Stream responses", value=True)
    mem_count = incident_count()
    st.caption(f"RAG memory: {mem_count} incident(s)")
...
user_input = st.chat_input("Ask about Spark, Delta Lake, SCD2, paste an error...")
...
if user_input:
    ...
    # RAG: inject relevant past incidents as extra system context
    rag_context = ""
    if rag_enabled and mem_count > 0:
        similar = retrieve_similar(user_input, k=2)
        if similar:
            parts = []
            for r in similar:
                parts.append(
            ...
            rag_context = "\n\nRelevant past incidents from RAG memory:\n" + "\n---\n".join(parts)
    ...
    with st.chat_message("assistant"):
        if not api_key:
            response = (
                "⚠️ No ANTHROPIC_API_KEY configured — I can't answer conversationally without it. "
                "Add your key in Streamlit → Settings → Secrets as `ANTHROPIC_API_KEY = 'sk-ant-...'`. "
                "The AI Agent and Code Assistant pages have rule-based fallbacks; the chat page needs a live API key."
            )
            ...
            try:
                if stream_enabled:
                    ...
                    if rag_context:
                        st.caption(f"📚 RAG context injected from {incident_count()} incident(s)")
                else:
                    resp = requests.post(
                        ANTHROPIC_API_URL,
                        headers={
